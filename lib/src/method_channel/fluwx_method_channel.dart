...
class MethodChannelFluwx extends FluwxPlatform {
  ...
  Future<bool> open(OpenType target) async {
    if (target is WeChatApp) {
      ...
      return await methodChannel.invokeMethod('subscribeMsg', target.arguments)??false;
    }
  ...
  Future<bool> authBy(AuthType which) async {
    if (which is NormalAuth) {
      return await methodChannel.invokeMethod('sendAuth', which.arguments)??false;
      ...
      return await methodChannel.invokeMethod('authByPhoneLogin', which.arguments)??false;
    }
  ...
  Future<bool> pay(PayType which) async {
    if (which is Payment) {
      return await methodChannel.invokeMethod('payWithFluwx', which.arguments)??false;
    } else if (which is HongKongWallet) {
      return await methodChannel.invokeMethod('payWithHongKongWallet', which.arguments)??false;
    }
